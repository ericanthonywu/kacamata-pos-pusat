...
$(function () {
      ...
      <% } %>
  {
    ...
    render: function (data, type, row) {
      ...
      return '<a href="' + (window.getAppPrefix ? window.getAppPrefix() : '') + '/pembayaran-pembelian/bayar/' + row.id + '" class="badge bg-danger text-decoration-none">❌ Belum Lunas</a>';
    }
  },
  {
    data: null,
    ...
    render: function (data, type, row) {
      ...
      if (window.userRole === 'admin') {
        if (row.status_bayar !== 'lunas') {
          html += '<a href="' + (window.getAppPrefix ? window.getAppPrefix() : '') + '/pembayaran-pembelian/bayar/' + row.id + '" class="btn btn-sm btn-outline-success" title="Bayar"><i class="bi bi-wallet2"></i></a>';
        }
        html += '<a href="' + (window.getAppPrefix ? window.getAppPrefix() : '') + '/pembelian-retur/baru?pembelian_id=' + row.id + '" class="btn btn-sm btn-outline-warning" title="Retur"><i class="bi bi-arrow-return-left"></i></a>';
      }
      html += '</div>';
      return html;
    }
